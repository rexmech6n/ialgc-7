        ...
        <form onSubmit={handleSignIn} className="z-50 pt-36 flex flex-1 flex-col mx-3 items-center justify-center">
            {!isSent && (
                <>
                    ...
                    <button disabled={loading || !!error || isSent} className="transition-colors duration-200 text-sm xsm:text-base mt-3 bg-red-950 py-3 px-5 rounded-[15px] disabled:bg-gray-500 disabled:text-gray-400">Doğrula</button>
                </>
            )}
            {isSent && <AuthSent />}
        </form>
